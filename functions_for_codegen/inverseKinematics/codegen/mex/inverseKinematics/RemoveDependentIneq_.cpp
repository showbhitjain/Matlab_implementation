                          ...
                          d_struct_T &qrmanager, g_struct_T &memspace)
{
  ...
  if ((workingset.nWConstr[2] + workingset.nWConstr[3]) +
    ...
    for (idx = 0; idx < nFixedConstr; idx++) {
      ...
      if ((idx + 1 < 1) || (idx + 1 > i)) {
        emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &bc_emlrtBCI,
                                      (emlrtConstCTX)&sp);
      }
    ...
    for (idx = idxDiag; idx <= TYPE; idx++) {
      i = qrmanager.jpvt.size(0);
      if ((idx < 1) || (idx > i)) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, i, &bc_emlrtBCI,
                                      (emlrtConstCTX)&sp);
      }
    ...
    for (idxDiag = 0; idxDiag < TYPE; idxDiag++) {
      st.site = &kg_emlrtRSI;
      if (nVar >= 1) {
        n_t = (ptrdiff_t)nVar;
        incx_t = (ptrdiff_t)1;
    ...
    for (idx = workingset.nActiveConstr; idx > nVar; idx--) {
      ...
      if ((nDepIneq < 1) || (nDepIneq > i)) {
        emlrtDynamicBoundsCheckR2012b(nDepIneq, 1, i, &bc_emlrtBCI,
                                      (emlrtConstCTX)&sp);
      }
    ...
    if (idx <= workingset.nVar) {
      ...
      while ((!exitg1) && (idx > nFixedConstr)) {
        ...
        if ((idxDiag < 1) || (idxDiag > i)) {
          emlrtDynamicBoundsCheckR2012b(idxDiag, 1, i, &bc_emlrtBCI,
                                        (emlrtConstCTX)&sp);
        }
    ...
    for (idx = nDepIneq; idx >= 1; idx--) {
      ...
      if ((idxDiag < 1) || (idxDiag > i)) {
        emlrtDynamicBoundsCheckR2012b(idxDiag, 1, i, &vb_emlrtBCI, &st);
      }
      TYPE = workingset.Wid[idxDiag - 1];
      i = workingset.Wlocalidx.size(0);
      if (idxDiag > i) {
        emlrtDynamicBoundsCheckR2012b(idxDiag, 1, i, &vb_emlrtBCI, &st);
      }
      i = workingset.Wid[idxDiag - 1];
      if ((i < 1) || (i > 6)) {
        emlrtDynamicBoundsCheckR2012b(i, 1, 6, &wb_emlrtBCI, &st);
      }
      i = workingset.isActiveConstr.size(0);
      nVar = (workingset.isActiveIdx[workingset.Wid[idxDiag - 1] - 1] +
              workingset.Wlocalidx[idxDiag - 1]) -
             1;
      if ((nVar < 1) || (nVar > i)) {
        emlrtDynamicBoundsCheckR2012b(nVar, 1, i, &vb_emlrtBCI, &st);
      }
      ...
      workingset.nActiveConstr--;
      if ((TYPE < 1) || (TYPE > 5)) {
        emlrtDynamicBoundsCheckR2012b(TYPE, 1, 5, &yb_emlrtBCI, &st);
      }
