                   ...
                   const array<real_T, 1U> &f, const array<real_T, 1U> &x)
{
  ...
  case 5:
    if ((obj.nvar < 1) || (obj.nvar > x.size(0))) {
      emlrtDynamicBoundsCheckR2012b(obj.nvar, 1, x.size(0), &nc_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
  ...
  default: {
    ...
    for (int32_T idx{a_tmp}; idx <= b_tmp; idx++) {
      ...
      if ((idx < 1) || (idx > x.size(0))) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, x.size(0), &oc_emlrtBCI, &st);
      }
      if (idx > workspace_tmp) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, workspace_tmp, &oc_emlrtBCI, &st);
      }
