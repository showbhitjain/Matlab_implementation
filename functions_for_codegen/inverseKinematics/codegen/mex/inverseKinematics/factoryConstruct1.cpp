...
void factoryConstruct(const emlrtStack &sp, const anonymous_function &objfun,
                      int32_T nVar, const array<real_T, 1U> &lb,
                      const array<real_T, 1U> &ub, l_struct_T &obj)
{
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack st;
  ...
  obj.hasLB.set_size(&gb_emlrtRTEI, &sp, nVar);
  obj.hasUB.set_size(&gb_emlrtRTEI, &sp, nVar);
  ...
  case 0U: {
    ...
    while ((!b) && (idx <= nVar)) {
      if ((idx < 1) || (idx > lb.size(0))) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, lb.size(0), &b_emlrtBCI, &st);
      }
      if (idx > obj.hasLB.size(0)) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, obj.hasLB.size(0), &b_emlrtBCI,
                                      &st);
      }
      d = lb[idx - 1];
      obj.hasLB[idx - 1] =
      ...
      if (idx > ub.size(0)) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, ub.size(0), &b_emlrtBCI, &st);
      }
      if (idx > obj.hasUB.size(0)) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, obj.hasUB.size(0), &b_emlrtBCI,
                                      &st);
      }
      d = ub[idx - 1];
      obj.hasUB[idx - 1] =
