               ...
               k_struct_T &qpoptions)
{
  ...
  for (idx = 0; idx < nVarOrig; idx++) {
    i = Hessian.size(0);
    if ((idx + 1 < 1) || (idx + 1 > i)) {
      emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &od_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
    i = Hessian.size(1);
    if (idx + 1 > i) {
  ...
  if (b_TrialState.sqpIterations <= 1) {
    ...
    if ((idx_max < 1) || (idx_max > grad.size(0))) {
      emlrtDynamicBoundsCheckR2012b(idx_max, 1, grad.size(0), &od_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
    ...
    if ((idx_max < 1) || (idx_max > i)) {
      emlrtDynamicBoundsCheckR2012b(idx_max, 1, i, &od_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
  ...
  for (idx = 0; idx < idx_max; idx++) {
    ...
    tmp_tmp = ((i1 - (idx_max << 1)) + idx) + 1;
    if ((tmp_tmp < 1) || (tmp_tmp > i)) {
      emlrtDynamicBoundsCheckR2012b(tmp_tmp, 1, i, &ub_emlrtBCI, &b_st);
    }
    tf = WorkingSet.isActiveConstr[tmp_tmp - 1];
    b_st.site = &yi_emlrtRSI;
    i = WorkingSet.isActiveConstr.size(0);
    i1 = ((i1 - idx_max) + idx) + 1;
    if ((i1 < 1) || (i1 > i)) {
  ...
  if (b_TrialState.state != -6) {
    ...
    rho = (b_TrialState.fstar - rho * qpfvalLinearExcess) -
          beta / 2.0 * qpfvalQuadExcess;
    st.site = &wi_emlrtRSI;
    qpfvalLinearExcess = b_MeritFunction.penaltyParam;
    if (mEq < 1) {
      qpfvalQuadExcess = 0.0;
    } else {
      n_t = (ptrdiff_t)mEq;
      incx_t = (ptrdiff_t)1;
      qpfvalQuadExcess = dasum(&n_t, &b_TrialState.cEq.data[0], &incx_t);
    }
    ...
    beta = (qpfvalQuadExcess + beta) - b_MeritFunction.linearizedConstrViol;
    ...
    if (qpfvalLinearExcess < b_MeritFunction.penaltyParam) {
      b_MeritFunction.phi =
          b_TrialState.sqpFval + qpfvalLinearExcess * qpfvalQuadExcess;
      if ((b_MeritFunction.initFval +
           qpfvalLinearExcess * b_MeritFunction.initConstrViolationEq) -
              b_MeritFunction.phi >
    ...
    b_MeritFunction.phiPrimePlus = muDoubleScalarMin(
        rho - b_MeritFunction.penaltyParam * qpfvalQuadExcess, 0.0);
    ...
    for (idx = idx_max; idx <= mFiniteLB; idx++) {
      i = WorkingSet.Wlocalidx.size(0);
      if ((idx < 1) || (idx > i)) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, i, &od_emlrtBCI,
                                      (emlrtConstCTX)&sp);
      }
  ...
  while ((idx > idx_max + mEq) && (nActiveLBArtificial > 0)) {
    ...
    if (i == 4) {
      ...
      if (i > mFiniteLB) {
        ...
        beta = b_TrialState.lambda[tmp_tmp];
        i1 = b_TrialState.lambda.size(0);
        if ((WorkingSet.nActiveConstr < 1) || (WorkingSet.nActiveConstr > i1)) {
          emlrtDynamicBoundsCheckR2012b(WorkingSet.nActiveConstr, 1, i1,
                                        &qd_emlrtBCI, &st);
        }
        b_TrialState.lambda[tmp_tmp] = 0.0;
        i1 = b_TrialState.lambda.size(0);
        if (idx > i1) {
          emlrtDynamicBoundsCheckR2012b(idx, 1, i1, &qd_emlrtBCI, &st);
        }
        b_TrialState.lambda[idx - 1] = beta;
        b_st.site = &aj_emlrtRSI;
        i1 = WorkingSet.Wid.size(0);
        if (idx > i1) {
          emlrtDynamicBoundsCheckR2012b(idx, 1, i1, &vb_emlrtBCI, &b_st);
        }
        ...
        WorkingSet.nActiveConstr = tmp_tmp;
  ...
  if (WorkingSet.nWConstr[4] > 0) {
    ...
    for (idx = 0; idx < idx_max; idx++) {
      ...
      if ((i1 < 1) || (i1 > i)) {
        emlrtDynamicBoundsCheckR2012b(i1, 1, i, &gb_emlrtBCI, &st);
      }
      i = WorkingSet.isActiveConstr.size(0);
      tmp_tmp = WorkingSet.isActiveIdxNormal[4] + idx;
      if ((tmp_tmp < 1) || (tmp_tmp > i)) {
        emlrtDynamicBoundsCheckR2012b(tmp_tmp, 1, i, &gb_emlrtBCI, &st);
      }
      WorkingSet.isActiveConstr[tmp_tmp - 1] =
          WorkingSet.isActiveConstr[i1 - 1];
    }
  }
  for (i = 0; i < 5; i++) {
