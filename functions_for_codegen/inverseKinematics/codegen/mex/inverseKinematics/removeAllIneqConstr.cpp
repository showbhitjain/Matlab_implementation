...
void removeAllIneqConstr(const emlrtStack &sp, j_struct_T &obj)
{
  ...
  for (int32_T idx_global{idxStartIneq}; idx_global <= idxEndIneq;
    ...
    if ((idx_global < 1) || (idx_global > i)) {
      emlrtDynamicBoundsCheckR2012b(idx_global, 1, i, &lc_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
    ...
    if ((i < 1) || (i > 6)) {
      emlrtDynamicBoundsCheckR2012b(obj.Wid[idx_global - 1], 1, 6, &mc_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
    i1 = obj.isActiveConstr.size(0);
    i = (obj.isActiveIdx[i - 1] + obj.Wlocalidx[idx_global - 1]) - 1;
    if ((i < 1) || (i > i1)) {
      emlrtDynamicBoundsCheckR2012b(i, 1, i1, &lc_emlrtBCI, (emlrtConstCTX)&sp);
    }
