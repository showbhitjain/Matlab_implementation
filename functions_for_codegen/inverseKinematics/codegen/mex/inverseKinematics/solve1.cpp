...
void solve(const emlrtStack &sp, const e_struct_T &obj, array<real_T, 1U> &rhs)
{
  ptrdiff_t incx_t;
  ptrdiff_t lda_t;
  ptrdiff_t n_t;
  ...
  for (int32_T idx{0}; idx < b; idx++) {
    ...
    i = rhs.size(0);
    if ((idx + 1 < 1) || (idx + 1 > i)) {
      emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &ed_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
    i = obj.FMat.size(0) * obj.FMat.size(1);
    i1 = (idx + obj.ldm * idx) + 1;
    if ((i1 < 1) || (i1 > i)) {
      emlrtDynamicBoundsCheckR2012b(i1, 1, i, &ed_emlrtBCI, (emlrtConstCTX)&sp);
    }
    i = rhs.size(0);
    if (idx + 1 > i) {
      emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &ed_emlrtBCI,
