    ...
    coder::array<double, 1U> &optimal_joint_velocity, double *Exit_Flag)
{
  ...
  if ((!pd_.is_first_step_not_empty) || (!pd_.starting_joint_vel_not_empty)) {
    ...
    if (static_cast<int>(joint_values.size(0) < 400)) {
...
#pragma omp parallel for num_threads(                                          \
  ...
  if (configInput->applyVelocityDamper) {
    ...
    if (jointminvalues.size(0) == joint_values.size(0)) {
      ...
      if (static_cast<int>(jointminvalues.size(0) < 400)) {
        for (int k{0}; k < loop_ub; k++) {
...
#pragma omp parallel for num_threads(                                          \
    ...
    sizes_idx_1 = (min_dist.size(0) < 400);
    if (sizes_idx_1) {
      for (int k{0}; k < nx; k++) {
...
#pragma omp parallel for num_threads(                                          \
    ...
    if (jointmaxvalues.size(0) == joint_values.size(0)) {
      ...
      if (static_cast<int>(jointmaxvalues.size(0) < 400)) {
        for (int k{0}; k < loop_ub; k++) {
...
#pragma omp parallel for num_threads(                                          \
    ...
    if (static_cast<int>(min_dist.size(0) < 400)) {
      for (int k{0}; k < nx; k++) {
...
#pragma omp parallel for num_threads(                                          \
    4 > omp_get_max_threads() ? omp_get_max_threads()                          \
                              : 4) private(c_varargin_1, c_varargin_2)
  ...
  if (configInput->applyInequalityConstraints) {
    ...
    if ((J_g.size(0) != 0) && (J_g.size(1) != 0) && (b_g.size(0) != 0)) {
      ...
      if (configInput->applyVelocityDamper) {
        ...
        loop_ub = b_g.size(0);
        if (static_cast<int>(b_g.size(0) < 400)) {
          for (int k{0}; k < loop_ub; k++) {
            min_dist[k] = b_g[k];
          }
        } else {
...
#pragma omp parallel for num_threads(                                          \
  ...
  if (!slackIsUsed) {
    ...
    if (configInput->applyVelocityDamper) {
      ...
      if (static_cast<int>(joint_min_vel.size(0) < 400)) {
        for (int k{0}; k < loop_ub; k++) {
...
#pragma omp parallel for num_threads(                                          \
    4 > omp_get_max_threads() ? omp_get_max_threads() : 4)
      ...
      if (static_cast<int>(joint_max_vel.size(0) < 400)) {
        for (int k{0}; k < loop_ub; k++) {
...
#pragma omp parallel for num_threads(                                          \
    4 > omp_get_max_threads() ? omp_get_max_threads() : 4)
      ...
      if (varargin_1.size(0) == joint_min_vel.size(0)) {
        ...
        loop_ub = varargin_1.size(0);
        if (static_cast<int>(varargin_1.size(0) < 400)) {
          for (int k{0}; k < loop_ub; k++) {
            b_varargin_1 = varargin_1[k];
...
#pragma omp parallel for num_threads(                                          \
    4 > omp_get_max_threads() ? omp_get_max_threads()                          \
      ...
      if (static_cast<int>(varargin_2.size(0) < 400)) {
        for (int k{0}; k < loop_ub; k++) {
          lb[k] = varargin_2[k];
        }
      } else {
...
#pragma omp parallel for num_threads(                                          \
    ...
    loop_ub = bp.size(0);
    if (static_cast<int>(bp.size(0) < 400)) {
      for (int k{0}; k < loop_ub; k++) {
        min_dist[k] = bp[k];
      }
    } else {
...
#pragma omp parallel for num_threads(                                          \
  ...
  if (slackIsUsed) {
    ...
    if (static_cast<int>(pd_.starting_joint_vel.size(0) < 400)) {
      for (int k{0}; k < loop_ub; k++) {
...
#pragma omp parallel for num_threads(                                          \
  ...
  if (static_cast<int>(optimal_joint_velocity.size(0) < 400)) {
    for (int k{0}; k < loop_ub; k++) {
    ...
    const coder::array<double, 1U> &q_vel)
{
  ...
  if (slackIsUsed) {
    ...
    if (static_cast<int>(b_loop_ub < 400)) {
