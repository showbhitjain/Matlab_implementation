...
int32_T compressBounds(const emlrtStack &sp, int32_T nVar,
                       ...
                       array<int32_T, 1U> &indexFixed, const real_T lb_data[],
    ...
    if (lb_size != 0) {
      ...
      for (int32_T idx{0}; idx < nVar; idx++) {
        ...
        if ((idx + 1 < 1) || (idx + 1 > lb_size)) {
          emlrtDynamicBoundsCheckR2012b(idx + 1, 1, lb_size, &lb_emlrtBCI,
                                        (emlrtConstCTX)&sp);
        }
        d = lb_data[idx];
        guard1 = false;
        if ((!muDoubleScalarIsInf(d)) && (!muDoubleScalarIsNaN(d))) {
          ...
          if (idx + 1 > ub_size) {
            emlrtDynamicBoundsCheckR2012b(idx + 1, 1, ub_size, &lb_emlrtBCI,
                                          (emlrtConstCTX)&sp);
          }
          if (muDoubleScalarAbs(d - ub_data[idx]) < 1.8967E-5) {
            mFixed++;
            i = indexFixed.size(0);
