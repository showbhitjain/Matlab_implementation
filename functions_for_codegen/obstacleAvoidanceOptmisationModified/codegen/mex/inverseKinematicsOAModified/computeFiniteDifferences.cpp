    ...
    const array<real_T, 1U> &runTimeOptions_TypicalX)
{
  ...
  while ((!exitg1) && (idx <= obj.nVar - 1)) {
      ...
      if (obj.hasLB[idx]) {
        ...
        if (obj.hasUB[idx]) {
          if ((idx + 1 < 1) || (idx + 1 > lb.size(0))) {
            emlrtDynamicBoundsCheckR2012b(idx + 1, 1, lb.size(0), &rb_emlrtBCI,
                                          &st);
          }
          if ((idx + 1 < 1) || (idx + 1 > ub.size(0))) {
            emlrtDynamicBoundsCheckR2012b(idx + 1, 1, ub.size(0), &rb_emlrtBCI,
                                          &st);
          }
          lbDiff = deltaX;
    ...
    ubDiff = c_inverseKinematicsOAModified_a(
        d_st, obj.objfun.workspace.jacobi, obj.objfun.workspace.xd_eff_vel,
      ...
      if (!modifiedStep) {
        ...
        if ((!obj.hasBounds) || modifiedStep) {
          ...
          ubDiff = c_inverseKinematicsOAModified_a(
              ...
              obj.objfun.workspace.xd_eff_vel,
              obj.objfun.workspace.starting_joint_vel,
              obj.objfun.workspace.joint_values,
              obj.objfun.workspace.jointVelocityWeightMatrix,
