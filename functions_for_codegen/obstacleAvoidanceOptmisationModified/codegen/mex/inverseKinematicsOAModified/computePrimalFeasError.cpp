...
real_T computePrimalFeasError(const emlrtStack &sp, const array<real_T, 1U> &x,
                              int32_T mLinIneq, const array<real_T, 1U> &cIneq,
                              ...
                              const array<real_T, 2U> &ub)
{
  ...
  for (int32_T idx{0}; idx < mLinIneq; idx++) {
    if ((idx + 1 < 1) || (idx + 1 > cIneq.size(0))) {
      emlrtDynamicBoundsCheckR2012b(idx + 1, 1, cIneq.size(0), &ib_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
  ...
  for (int32_T idx{0}; idx < mLB; idx++) {
    ...
    if ((finiteLB[idx] < 1) || (finiteLB[idx] > lb.size(0))) {
      emlrtDynamicBoundsCheckR2012b(finiteLB[idx], 1, lb.size(0), &ib_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
  ...
  for (int32_T idx{0}; idx < mUB; idx++) {
    ...
    if ((finiteUB[idx] < 1) || (finiteUB[idx] > ub.size(0))) {
      emlrtDynamicBoundsCheckR2012b(finiteUB[idx], 1, ub.size(0), &ib_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
