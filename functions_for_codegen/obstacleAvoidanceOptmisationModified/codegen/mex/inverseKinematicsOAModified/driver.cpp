...
void driver(const emlrtStack &sp, array<real_T, 2U> &Hessian,
            const real_T bineq_data[], const real_T beq_data[],
            const real_T lb_data[], int32_T lb_size, const real_T ub_data[],
            int32_T ub_size, j_struct_T &b_TrialState,
            ...
            const i_struct_T &runTimeOptions)
{
  ...
  st.site = &gf_emlrtRSI;
  Flags.gradOK = test_exit(st, b_MeritFunction, fscales_lineq_constraint_size,
                           WorkingSet, b_TrialState, lb_data, lb_size, ub_data,
                           ub_size, runTimeOptions.MaxFunctionEvaluations,
  ...
  while (!Flags.done) {
    ...
    while (!(Flags.stepAccepted || Flags.failedLineSearch)) {
      if (Flags.stepType != 3) {
        ...
        internal::updateWorkingSetForNewQP(
            st, b_TrialState.xstarsqp, WorkingSet, mIneq,
            b_TrialState.cIneq.data, b_TrialState.cIneq.size[0], mEq,
            b_TrialState.cEq.data, b_TrialState.cEq.size[0], mLB, lb_data,
      ...
      st.site = &gf_emlrtRSI;
      Flags.stepAccepted =
          b_step(st, Flags.stepType, Hessian, lb_data, lb_size, ub_data,
                 ub_size, b_TrialState, b_MeritFunction, memspace, WorkingSet,
      ...
      if (Flags.stepAccepted) {
        ...
        internal::computeLinearResiduals(
            st, b_TrialState.xstarsqp, nVar, b_TrialState.cIneq.data,
            b_TrialState.cIneq.size[0], mIneq, WorkingSet.Aineq, bineq_data,
            WorkingSet.ldA, b_TrialState.cEq.data, b_TrialState.cEq.size[0],
            mEq, WorkingSet.Aeq, beq_data, WorkingSet.ldA);
        st.site = &gf_emlrtRSI;
        if (Flags.fevalOK) {
          ...
          constrViolationIneq = MeritFunction::computeConstrViolationIneq_(
              b_st, mIneq, b_TrialState.cIneq.data, b_TrialState.cIneq.size[0]);
      ...
      if ((Flags.stepType == 1) && Flags.stepAccepted && Flags.fevalOK &&
        ...
        constrViolationEq = linesearch(
            st, Flags.fevalOK, bineq_data, beq_data, WorkingSet.nVar,
            WorkingSet.ldA, WorkingSet.Aineq, WorkingSet.Aeq, b_TrialState,
    ...
    if (Flags.stepAccepted && (!Flags.failedLineSearch)) {
      ...
      for (int32_T idx{0}; idx < nVar; idx++) {
        ...
        if ((idx + 1 < 1) || (idx + 1 > i)) {
          emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &xb_emlrtBCI,
                                        (emlrtConstCTX)&sp);
        }
        i = b_TrialState.delta_x.size(0);
        if (idx + 1 > i) {
      ...
      for (int32_T idx{0}; idx < mConstr; idx++) {
        ...
        y = b_TrialState.lambda.size[0];
        if (idx + 1 > y) {
          emlrtDynamicBoundsCheckR2012b(idx + 1, 1, y, &xb_emlrtBCI,
                                        (emlrtConstCTX)&sp);
        }
        if (idx + 1 > i) {
          emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &xb_emlrtBCI,
        ...
        constrViolationEq = b_TrialState.lambdasqp.data[idx];
        b_TrialState.lambdasqp.data[idx] =
            constrViolationEq +
            b_TrialState.steplength *
                (b_TrialState.lambda.data[idx] - constrViolationEq);
      }
      st.site = &gf_emlrtRSI;
      TrialState::saveState(st, b_TrialState);
      st.site = &gf_emlrtRSI;
      Flags.gradOK = utils::FiniteDifferences::computeFiniteDifferences(
          ...
          b_TrialState.grad, lb_data, lb_size, ub_data, ub_size,
          runTimeOptions.FiniteDifferenceStepSize, runTimeOptions.TypicalX);
      ...
      b_st.site = &jj_emlrtRSI;
      b_y.set_size(&rb_emlrtRTEI, &b_st, b_TrialState.cIneq.size[0]);
      y = b_TrialState.cIneq.size[0];
      for (i = 0; i < y; i++) {
        b_y[i] = b_TrialState.cIneq.data[i];
      }
      if (b_TrialState.mIneq >= 1) {
        n_t = (ptrdiff_t)b_TrialState.mIneq;
        incx_t = (ptrdiff_t)1;
        incy_t = (ptrdiff_t)1;
        dcopy(&n_t, &b_TrialState.cIneq_old.data[0], &incx_t, &(b_y.data())[0],
              &incy_t);
      }
      b_TrialState.cIneq.size[0] = b_y.size(0);
      y = b_y.size(0);
      for (i = 0; i < y; i++) {
        b_TrialState.cIneq.data[i] = b_y[i];
      }
      b_st.site = &jj_emlrtRSI;
      b_y.set_size(&rb_emlrtRTEI, &b_st, b_TrialState.cEq.size[0]);
    ...
    st.site = &gf_emlrtRSI;
    b_test_exit(st, Flags, memspace, b_MeritFunction,
                fscales_lineq_constraint_size, WorkingSet, b_TrialState,
                b_QRManager, lb_data, lb_size, ub_data, ub_size,
