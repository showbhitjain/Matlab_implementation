            ...
            int32_T runTimeOptions_MaxIterations)
{
  ...
  if (H.size(0) != H.size(1)) {
    emlrtErrorWithMessageIdR2018a(
        &b_st, &q_emlrtRTEI, "Coder:toolbox:ValidateattributesexpectedSquare",
  ...
  if (workingset.probType == 3) {
    ...
    for (mEqFixed = 0; mEqFixed < startIdx; mEqFixed++) {
      ...
      if ((mEqFixed + 1 < 1) || (mEqFixed + 1 > i)) {
        emlrtDynamicBoundsCheckR2012b(mEqFixed + 1, 1, i, &ic_emlrtBCI, &st);
      }
  ...
  if (guard1) {
    ...
    if (solution.maxConstr > 1.8967E-5) {
      ...
      if ((nVarP1_tmp < 1) || (nVarP1_tmp > i)) {
        emlrtDynamicBoundsCheckR2012b(nVarP1_tmp, 1, i, &mc_emlrtBCI, &st);
      }
      ...
      iterate(b_st, H, f, solution, memspace, workingset, qrmanager,
              ...
              1.4901161193847657E-10, 1.8967E-5, runTimeOptions_MaxIterations);
      startIdx = (workingset.sizes[0] + workingset.sizes[1]) + 1;
      b_st.site = &bh_emlrtRSI;
      i = workingset.isActiveConstr.size[0];
      i1 = workingset.isActiveIdx[3] + workingset.sizes[3];
      if ((i1 - 1 < 1) || (i1 - 1 > i)) {
        emlrtDynamicBoundsCheckR2012b(i1 - 1, 1, i, &jc_emlrtBCI, &b_st);
      }
      if (workingset.isActiveConstr.data[i1 - 2]) {
        ...
        while ((!exitg1) && (startIdx <= workingset.nActiveConstr)) {
          ...
          if ((startIdx < 1) || (startIdx > i)) {
            emlrtDynamicBoundsCheckR2012b(startIdx, 1, i, &mc_emlrtBCI, &st);
          }
          if (workingset.Wid.data[startIdx - 1] == 4) {
            i = workingset.Wlocalidx.size[0];
            if (startIdx > i) {
              emlrtDynamicBoundsCheckR2012b(startIdx, 1, i, &mc_emlrtBCI, &st);
            }
            i = workingset.Wlocalidx.data[startIdx - 1];
            if (i == workingset.sizes[3]) {
              b_st.site = &bh_emlrtRSI;
              i1 = workingset.Wid.size[0];
              if (startIdx > i1) {
                emlrtDynamicBoundsCheckR2012b(startIdx, 1, i1, &kc_emlrtBCI,
                                              &b_st);
              }
      ...
      while ((startIdx > mEqFixed) && (startIdx > nVar_tmp)) {
        ...
        i = workingset.Wid.size[0];
        if ((startIdx < 1) || (startIdx > i)) {
          emlrtDynamicBoundsCheckR2012b(startIdx, 1, i, &kc_emlrtBCI, &b_st);
        }
        TYPE_tmp = workingset.Wid.data[startIdx - 1];
        i = workingset.Wlocalidx.size[0];
        if (startIdx > i) {
          emlrtDynamicBoundsCheckR2012b(startIdx, 1, i, &kc_emlrtBCI, &b_st);
        }
        if ((TYPE_tmp < 1) || (TYPE_tmp > 6)) {
          emlrtDynamicBoundsCheckR2012b(workingset.Wid.data[startIdx - 1], 1, 6,
                                        &lc_emlrtBCI, &b_st);
        }
        i = workingset.isActiveConstr.size[0];
        i1 = (workingset.isActiveIdx[TYPE_tmp - 1] +
              workingset.Wlocalidx.data[startIdx - 1]) -
             1;
        if ((i1 < 1) || (i1 > i)) {
        ...
        workingset.nActiveConstr--;
        if (TYPE_tmp > 5) {
          emlrtDynamicBoundsCheckR2012b(6, 1, 5, &nc_emlrtBCI, &b_st);
        }
      ...
      if (solution.state != 0) {
        ...
        solution.maxConstr =
            WorkingSet::maxConstraintViolation(st, workingset, solution.xstar);
        if (solution.maxConstr > 1.8967E-5) {
          r.set_size(&wb_emlrtRTEI, &sp, solution.lambda.size[0]);
          startIdx = solution.lambda.size[0];
