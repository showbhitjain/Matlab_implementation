                      ...
                      j_struct_T &obj)
{
  ...
  obj.sqpFval = 0.0;
  obj.sqpFval_old = 0.0;
  obj.xstarsqp.set_size(&db_emlrtRTEI, &sp, x0.size(0));
  obj.xstarsqp_old.set_size(&db_emlrtRTEI, &sp, x0.size(0));
  obj.cIneq.size[0] = mIneq;
  obj.cIneq_old.size[0] = mIneq;
  obj.cEq.size[0] = mEq;
  obj.cEq_old.size[0] = mEq;
  obj.grad.set_size(&db_emlrtRTEI, &sp, nVarMax);
  obj.grad_old.set_size(&db_emlrtRTEI, &sp, nVarMax);
  ...
  if (mConstrMax - 1 >= 0) {
    std::memset(&obj.lambdasqp.data[0], 0,
                static_cast<uint32_T>(mConstrMax) * sizeof(real_T));
  }
  obj.lambdaStopTest.size[0] = mConstrMax;
  obj.lambdaStopTestPrev.size[0] = mConstrMax;
  obj.steplength = 1.0;
  obj.delta_x.set_size(&db_emlrtRTEI, &sp, nVarMax);
  for (int32_T i{0}; i < nVarMax; i++) {
    obj.delta_x[i] = 0.0;
  }
  obj.socDirection.set_size(&db_emlrtRTEI, &sp, nVarMax);
  obj.workingset_old.size[0] = mConstrMax;
  obj.gradLag.set_size(&db_emlrtRTEI, &sp, nVarMax);
  obj.delta_gradLag.set_size(&db_emlrtRTEI, &sp, nVarMax);
  obj.xstar.set_size(&db_emlrtRTEI, &sp, nVarMax);
  obj.fstar = 0.0;
  ...
  obj.maxConstr = 0.0;
  obj.iterations = 0;
  obj.searchDir.set_size(&db_emlrtRTEI, &sp, nVarMax);
}
