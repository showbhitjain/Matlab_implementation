...
void factoryConstruct(const emlrtStack &sp, const anonymous_function &objfun,
                      int32_T nVar, const real_T lb_data[], int32_T lb_size,
                      const real_T ub_data[], int32_T ub_size, l_struct_T &obj)
{
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack st;
  ...
  obj.hasLB.set_size(&eb_emlrtRTEI, &sp, nVar);
  obj.hasUB.set_size(&eb_emlrtRTEI, &sp, nVar);
  obj.FiniteDifferenceType = 0;
  st.site = &xb_emlrtRSI;
  b = false;
  idx = 1;
  switch (static_cast<uint32_T>(ub_size == 0) << 1 |
          static_cast<uint32_T>(lb_size == 0)) {
  case 0U: {
    real_T d;
    while ((!b) && (idx <= nVar)) {
      if (idx > lb_size) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, lb_size, &i_emlrtBCI, &st);
      }
      ...
      if (idx > ub_size) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, ub_size, &i_emlrtBCI, &st);
      }
