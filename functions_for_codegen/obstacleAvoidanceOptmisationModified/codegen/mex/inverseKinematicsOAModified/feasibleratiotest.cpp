    ...
    boolean_T &newBlocking, int32_T &constrType, int32_T &constrIdx)
{
  ...
  if (workingset_nWConstr[2] < workingset_sizes[2]) {
    ...
    i = static_cast<uint8_T>(workingset_sizes[2]);
    for (int32_T idx{0}; idx < i; idx++) {
      int32_T i2;
      i1 = workspace.size(0) * workspace.size(1);
      i2 = (ldw + idx) + 1;
      if ((i2 < 1) || (i2 > i1)) {
        emlrtDynamicBoundsCheckR2012b(i2, 1, i1, &ne_emlrtBCI,
                                      (emlrtConstCTX)&sp);
      }
      alphaTemp = workspace[i2 - 1];
      if (alphaTemp > denomTol) {
        int32_T i3;
        st.site = &mi_emlrtRSI;
        i3 = workingset_isActiveIdx[2] + idx;
        if ((i3 < 1) || (i3 > workingset_isActiveConstr_size)) {
          emlrtDynamicBoundsCheckR2012b(i3, 1, workingset_isActiveConstr_size,
                                        &jc_emlrtBCI, &st);
        }
        if (!workingset_isActiveConstr_data[i3 - 1]) {
          if (idx + 1 > i1) {
            emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i1, &ne_emlrtBCI,
                                          (emlrtConstCTX)&sp);
          }
          if (i2 > i1) {
            emlrtDynamicBoundsCheckR2012b(i2, 1, i1, &ne_emlrtBCI,
      ...
      if (pk_corrected > denomTol) {
        ...
        if (!workingset_isActiveConstr_data[i1 - 1]) {
          ...
          ratio = (-solution_xstar[workingset_indexLB[idx] - 1] -
                  ...
                  phaseOneCorrectionX;
          alphaTemp =
              muDoubleScalarMin(muDoubleScalarAbs(ratio), 1.8967E-5 - ratio) /
              pk_corrected;
          if (alphaTemp < alpha) {
            alpha = alphaTemp;
            constrType = 4;
