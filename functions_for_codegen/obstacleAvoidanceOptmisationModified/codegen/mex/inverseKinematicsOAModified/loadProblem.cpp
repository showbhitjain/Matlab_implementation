                 ...
                 int32_T mLB, int32_T mUB, int32_T mFixed, int32_T mConstrMax)
{
  ...
  if (mIneq > 0) {
    ...
    i = static_cast<uint8_T>(mLinIneq);
    for (int32_T idx_col{0}; idx_col < i; idx_col++) {
      ...
      for (int32_T idx_row{0}; idx_row < k; idx_row++) {
        i1 = Aineq_size[0] * Aineq_size[1];
        i2 = (idx_col + mLinIneq * idx_row) + 1;
        if ((i2 < 1) || (i2 > i1)) {
        ...
        obj.Aineq[i3 - 1] = Aineq_data[i2 - 1];
      }
    }
  }
  if (mEq > 0) {
