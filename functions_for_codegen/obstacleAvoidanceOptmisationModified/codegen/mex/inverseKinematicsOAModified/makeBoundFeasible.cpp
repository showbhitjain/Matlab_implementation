...
void makeBoundFeasible(const emlrtStack &sp, array<real_T, 1U> &xk,
                       const d_struct_T &WorkingSet, const real_T lb_data[],
  ...
  if (lb_size != 0) {
    if (ub_size == 0) {
      ...
      for (int32_T idx{0}; idx <= mLB; idx++) {
        ...
        if ((idx + 1 < 1) || (idx + 1 > i)) {
          emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &be_emlrtBCI,
                                        (emlrtConstCTX)&sp);
        }
        i = xk.size(0);
      ...
      for (int32_T idx{0}; idx <= mLB; idx++) {
        ...
        if (-xk[WorkingSet.indexLB[idx] - 1] > d) {
          if ((WorkingSet.indexLB[idx] < 1) ||
              (WorkingSet.indexLB[idx] > ub_size)) {
            emlrtDynamicBoundsCheckR2012b(WorkingSet.indexLB[idx], 1, ub_size,
                                          &be_emlrtBCI, (emlrtConstCTX)&sp);
          }
          if (muDoubleScalarIsInf(ub_data[WorkingSet.indexLB[idx] - 1])) {
            i = WorkingSet.lb.size(0);
            if ((WorkingSet.indexLB[idx] < 1) ||
      ...
      for (int32_T idx{0}; idx <= mUB; idx++) {
        ...
        if (xk[WorkingSet.indexUB[idx] - 1] > d) {
          if ((WorkingSet.indexUB[idx] < 1) ||
              (WorkingSet.indexUB[idx] > lb_size)) {
            emlrtDynamicBoundsCheckR2012b(WorkingSet.indexUB[idx], 1, lb_size,
                                          &be_emlrtBCI, (emlrtConstCTX)&sp);
          }
          if (muDoubleScalarIsInf(lb_data[WorkingSet.indexUB[idx] - 1])) {
            i = WorkingSet.ub.size(0);
            if ((WorkingSet.indexUB[idx] < 1) ||
