...
void modifyOverheadRegularized_(const emlrtStack &sp, d_struct_T &obj)
{
  ...
  for (int32_T idx_col{0}; idx_col < i; idx_col++) {
    ...
    for (int32_T idx_row{offsetIneq_tmp}; idx_row <= b; idx_row++) {
      i1 = obj.Aineq.size(0);
      colOffsetATw = idx_row + colOffsetAineq;
      if ((colOffsetATw < 1) || (colOffsetATw > i1)) {
        emlrtDynamicBoundsCheckR2012b(colOffsetATw, 1, i1, &wb_emlrtBCI,
                                      (emlrtConstCTX)&sp);
      }
      obj.Aineq[colOffsetATw - 1] = 0.0;
    }
    i1 = obj.Aineq.size(0);
    colOffsetATw = b_tmp + colOffsetAineq;
    if ((colOffsetATw < 1) || (colOffsetATw > i1)) {
      emlrtDynamicBoundsCheckR2012b(colOffsetATw, 1, i1, &wb_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
    obj.Aineq[colOffsetATw - 1] = -1.0;
    a = b_tmp + 1;
    b = obj.nVar;
    st.site = &df_emlrtRSI;
  ...
  if (obj.nWConstr[4] > 0) {
    ...
    for (colOffsetAeq = 0; colOffsetAeq < b; colOffsetAeq++) {
      ...
      i = obj.isActiveConstr.size[0];
      colOffsetATw = (obj.isActiveIdxRegularized[4] + colOffsetAeq) + 1;
      if ((colOffsetATw < 1) || (colOffsetATw > i)) {
        emlrtDynamicBoundsCheckR2012b(colOffsetATw, 1, i, &wb_emlrtBCI,
                                      (emlrtConstCTX)&sp);
      }
      obj.isActiveConstr.data[colOffsetATw - 1] =
          obj.isActiveConstr.data[i1 - 1];
    }
  }
  a = obj.isActiveIdx[4];
  ...
  for (colOffsetAeq = a; colOffsetAeq <= b; colOffsetAeq++) {
    ...
    obj.isActiveConstr.data[colOffsetAeq - 1] = false;
  }
  ...
  for (int32_T idx_col{colOffsetAineq}; idx_col <= b; idx_col++) {
    ...
    if (obj.Wid.data[idx_col - 1] == 3) {
      ...
      b_tmp = offsetIneq_tmp + obj.Wlocalidx.data[idx_col - 1];
