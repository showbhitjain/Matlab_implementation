               ...
               k_struct_T &qpoptions)
{
  ...
  if (b_TrialState.sqpIterations <= 1) {
    ...
    rho = muDoubleScalarAbs(b_TrialState.lambdasqp.data[idx_max - 1]);
  }
  ...
  for (idx = 0; idx < idx_max; idx++) {
    ...
    b_tf = WorkingSet.isActiveConstr.data[i1 - 1];
  ...
  for (idx = 0; idx < i; idx++) {
    ...
    i2 = ((WorkingSet.isActiveIdx[3] + mFiniteLB) - nArtificial) + idx;
    if ((i2 < 1) || (i2 > i1)) {
      emlrtDynamicBoundsCheckR2012b(i2, 1, i1, &jc_emlrtBCI, &b_st);
    }
    tf = WorkingSet.isActiveConstr.data[i2 - 1];
    i1 = memspace.workspace_int.size(0);
    i2 = (idx + nArtificial_tmp_tmp_tmp) + 1;
    if ((i2 < 1) || (i2 > i1)) {
      emlrtDynamicBoundsCheckR2012b(i2, 1, i1, &de_emlrtBCI, &st);
    }
    memspace.workspace_int[i2 - 1] = tf;
  ...
  if (b_TrialState.state != -6) {
    ...
    rho = MeritFunction::computeConstrViolationIneq_(
    ...
    for (idx = 0; idx < mEq; idx++) {
      ...
      b_TrialState.lambda.data[i1 - 1] *= static_cast<real_T>(tf);
    }
    idx_max = WorkingSet.isActiveIdx[2];
    nArtificial = WorkingSet.nActiveConstr;
    st.site = &wi_emlrtRSI;
    ...
    for (idx = idx_max; idx <= nArtificial; idx++) {
  ...
  while ((idx > mFiniteLB + mEq) && (nActiveLBArtificial > 0)) {
    ...
    if (WorkingSet.Wid.data[idx - 1] == 4) {
      ...
      if (i > idx_max) {
        ...
        beta = b_TrialState.lambda.data[nArtificial];
        i1 = b_TrialState.lambda.size[0];
        if ((WorkingSet.nActiveConstr < 1) || (WorkingSet.nActiveConstr > i1)) {
          emlrtDynamicBoundsCheckR2012b(WorkingSet.nActiveConstr, 1, i1,
                                        &ee_emlrtBCI, &st);
        }
        b_TrialState.lambda.data[nArtificial] = 0.0;
        i1 = b_TrialState.lambda.size[0];
        if (idx > i1) {
          emlrtDynamicBoundsCheckR2012b(idx, 1, i1, &ee_emlrtBCI, &st);
        }
        b_TrialState.lambda.data[idx - 1] = beta;
  ...
  if (WorkingSet.nWConstr[4] > 0) {
    ...
    for (idx = 0; idx < idx_max; idx++) {
      ...
      WorkingSet.isActiveConstr.data[i2 - 1] =
          WorkingSet.isActiveConstr.data[i1 - 1];
    }
  }
  for (i = 0; i < 5; i++) {
