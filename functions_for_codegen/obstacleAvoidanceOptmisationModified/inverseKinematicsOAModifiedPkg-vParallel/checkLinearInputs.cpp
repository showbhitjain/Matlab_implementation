                         ...
                         const array<double, 2U> &ub)
{
  ...
  if ((Aineq.size(0) != 0) && (Aineq.size(1) != 0)) {
    b_Aineq = Aineq.size(0) * Aineq.size(1);
    r.set_size(b_Aineq);
    if (static_cast<int>(b_Aineq < 1600)) {
      for (int i{0}; i < b_Aineq; i++) {
...
#pragma omp parallel for num_threads(                                          \
  ...
  if ((bineq.size(0) != 0) && (bineq.size(1) != 0)) {
    ...
    b_Aineq = bineq.size(0);
    if (static_cast<int>(bineq.size(0) < 1600)) {
      for (int i{0}; i < b_Aineq; i++) {
...
#pragma omp parallel for num_threads(                                          \
  ...
  if ((beq_size[0] != 0) && (beq_size[1] != 0)) {
    ...
    b_Aineq = beq_size[0];
    for (i1 = 0; i1 < b_Aineq; i1++) {
      double d;
      d = beq_data[i1];
      r[i1] = ((!std::isinf(d)) && (!std::isnan(d)));
    }
    if (!all(r)) {
...
#pragma omp parallel for num_threads(                                          \
  ...
  b_Aineq = 1;
  exitg1 = false;
  while ((!exitg1) && (b_Aineq <= b_lb.size(0))) {
    if (b_lb[b_Aineq - 1]) {
      y = true;
      exitg1 = true;
    } else {
...
#pragma omp parallel for num_threads(                                          \
  ...
  if ((!y) && (ub.size(0) != 0)) {
    ...
    b_Aineq = 0;
    exitg1 = false;
    while ((!exitg1) && (b_Aineq <= nVar - 1)) {
      if ((b_Aineq + 1 < 1) || (b_Aineq + 1 > lb.size(0))) {
        rtDynamicBoundsError(b_Aineq + 1, 1, lb.size(0), w_emlrtBCI);
      }
      if ((b_Aineq + 1 < 1) || (b_Aineq + 1 > ub.size(0))) {
        rtDynamicBoundsError(b_Aineq + 1, 1, ub.size(0), w_emlrtBCI);
      }
      if (lb[b_Aineq] > ub[b_Aineq]) {
        exitflag = -2.0;
        exitg1 = true;
      } else {
        b_Aineq++;
