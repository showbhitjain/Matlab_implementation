    ...
    double config_weightManipulability)
{
  ...
  if (config_useObjectiveJointAcceleration) {
    ...
    if (current_joint_velocity.size(0) == starting_joint_vel.size(0)) {
      ...
      if (static_cast<int>(current_joint_velocity.size(0) < 1600)) {
        for (i1 = 0; i1 < pipk; i1++) {
...
#pragma omp parallel for num_threads(                                          \
  ...
  if (config_useObjectiveManipulability) {
    ...
    i = (joint_values.size(0) < 1600);
    if (i) {
      for (i1 = 0; i1 < pipk; i1++) {
        y[i1] = joint_values[i1];
      }
    } else {
#pragma omp parallel for num_threads(                                          \
    16 > omp_get_max_threads() ? omp_get_max_threads() : 16)
...
#pragma omp parallel for num_threads(16 > omp_get_max_threads()                \
                                         ? omp_get_max_threads()               \
                                         : 16) private(i1, b_JJt, i2, i3, JJt)
    ...
    for (int c_i = 0; c_i < 7; c_i++) {
      ...
      for (i3 = 0; i3 < 6; i3++) {
        for (i2 = 0; i2 < 6; i2++) {
          b_JJt = 0.0;
          for (i1 = 0; i1 < 7; i1++) {
            b_JJt += J[i3 + 6 * i1] * H[(i2 + 6 * i1) + 42 * c_i];
          }
          JJt[i3 + 6 * i2] = b_JJt;
        }
      }
      b_JJt = 0.0;
      for (i1 = 0; i1 < 36; i1++) {
        b_JJt += JJt[i1] * inv_JJt[i1];
      }
      Jm_transposed[c_i] = t * b_JJt;
      //  Equation (11)
    }
