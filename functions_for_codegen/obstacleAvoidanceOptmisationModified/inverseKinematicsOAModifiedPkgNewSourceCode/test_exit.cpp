                 ...
                 int runTimeOptions_MaxFunctionEvaluations)
{
  ...
  if (!Flags.gradOK) {
    ...
    if (isFeasible &&
        (b_MeritFunction.nlpDualFeasError <= 0.001 * optimRelativeFactor) &&
        (b_MeritFunction.nlpComplError <= 0.001 * optimRelativeFactor)) {
      Flags.done = true;
      ...
      if (isFeasible && (b_TrialState.sqpFval < -1.0E+20)) {
        ...
        if (b_TrialState.sqpIterations > 0) {
          ...
          exitg1 = false;
          while ((!exitg1) && (idx <= nVar_tmp - 1)) {
            if (0.0001 * std::fmax(1.0, std::abs(b_TrialState.xstarsqp[idx])) <=
          ...
          if (dxTooSmall) {
            if (!isFeasible) {
              ...
              if (WorkingSet.nActiveConstr > 0) {
                ...
                if ((smax <= 0.001 * optimRelativeFactor) &&
                    (s <= 0.001 * optimRelativeFactor)) {
                  b_MeritFunction.nlpDualFeasError = smax;
                    ...
                    boolean_T &Flags_failedLineSearch, int &Flags_stepType)
{
  ...
  if (!Flags_gradOK) {
    ...
    if (isFeasible && (b_MeritFunction.nlpDualFeasError <= 0.001 * smax)) {
      Flags_done = true;
      b_TrialState.sqpExitFlag = 1;
