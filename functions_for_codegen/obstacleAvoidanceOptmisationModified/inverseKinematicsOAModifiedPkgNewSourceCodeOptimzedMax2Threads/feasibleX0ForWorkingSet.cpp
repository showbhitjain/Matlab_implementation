...
#pragma omp parallel for num_threads(                                          \
    2 > omp_get_max_threads() ? omp_get_max_threads() : 2) private(x)
...
#pragma omp parallel num_threads(                                              \
    2 > omp_get_max_threads() ? omp_get_max_threads() : 2) private(b_vPrime)
              {
                b_vPrime = rtMinusInf;
#pragma omp for nowait
                for (idx = 0; idx <= minmn; idx++) {
...
#pragma omp parallel num_threads(                                              \
    2 > omp_get_max_threads() ? omp_get_max_threads() : 2) private(vPrime, x)
              {
                vPrime = rtMinusInf;
#pragma omp for nowait
                for (idx = 0; idx <= ar; idx++) {
...
#pragma omp parallel num_threads(                                              \
    2 > omp_get_max_threads() ? omp_get_max_threads()                          \
                              : 2) private(c_constrViolation_minnormXPrime)
            {
