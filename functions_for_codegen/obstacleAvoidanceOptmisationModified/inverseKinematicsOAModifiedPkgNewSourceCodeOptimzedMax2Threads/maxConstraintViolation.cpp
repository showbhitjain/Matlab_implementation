...
#pragma omp parallel num_threads(                                              \
    2 > omp_get_max_threads() ? omp_get_max_threads() : 2) private(c_vPrime)
        {
          c_vPrime = rtMinusInf;
#pragma omp for nowait
          for (k = 0; k <= mIneq; k++) {
...
#pragma omp parallel num_threads(                                              \
    2 > omp_get_max_threads() ? omp_get_max_threads() : 2) private(e_vPrime)
      {
        e_vPrime = rtMinusInf;
#pragma omp for nowait
        for (idx = 0; idx < mFixed; idx++) {
