...
#pragma omp parallel for num_threads(                                          \
  ...
  for (idx = 0; idx <= b_mEq; idx++) {
    ...
    if (memspace.workspace_double[idx] <= 0.0) {
      ...
      if (memspace.workspace_double[idx] >= -1.8967E-5) {
        qpactiveset::WorkingSet::addBoundToActiveSetMatrix_(
...
#pragma omp parallel num_threads(                                              \
    2 > omp_get_max_threads() ? omp_get_max_threads()                          \
                              : 2) private(nActiveLBArtificialPrime, tf, b_tf)
    {
...
#pragma omp parallel num_threads(                                              \
    2 > omp_get_max_threads() ? omp_get_max_threads()                          \
                              : 2) private(b_nActiveLBArtificialPrime, tf)
    {
